...
if(e->interpolation_order==1){ // la décomp avec homo-box
...
if(e->interpolation_order==2){ // la décomp avec szeli et ripmap
	
	if(e->pd==3){
        apply_homo_final_period(e->img,img_f,w,h,f->w,f->h,H);
	}else{//suppose pd=1
        float *img3 = malloc(3*w*h*sizeof(float));
        for(int l = 0;l<3;l++){
            for(int i=0;i<w*h;i++){
        ...
        apply_homo_final_period(img3,img_f,w,h,f->w,f->h,H);
	}
	for(int i=0;i<3*(f->w)*(f->h);i++){(f->rgb)[i]=float_to_byte(img_f[i]);}
}
...
if(e->interpolation_order==3){ // la décomp avec szeli et ripmap
	
	if(e->pd==3){
        apply_homo_final_period_yaro(e->img,img_f,w,h,f->w,f->h,H);
	}else{//suppose pd=1
        float *img3 = malloc(3*w*h*sizeof(float));
        for(int l = 0;l<3;l++){
            for(int i=0;i<w*h;i++){
        ...
        apply_homo_final_period_yaro(img3,img_f,w,h,f->w,f->h,H);
	}
	for(int i=0;i<3*(f->w)*(f->h);i++){(f->rgb)[i]=float_to_byte(img_f[i]);}
}


if(e->interpolation_order==4){ // la décomp avec szeli et ripmap
	
	if(e->pd==3){
        apply_homo_final_yaro(e->img,img_f,w,h,f->w,f->h,H);
	}else{//suppose pd=1
        float *img3 = malloc(3*w*h*sizeof(float));
        for(int l = 0;l<3;l++){
            for(int i=0;i<w*h;i++){
        ...
        apply_homo_final_yaro(img3,img_f,w,h,f->w,f->h,H);
	}
	for(int i=0;i<3*(f->w)*(f->h);i++){(f->rgb)[i]=float_to_byte(img_f[i]);}
}

if(e->interpolation_order==5){ // le ripmap tout simplement !
	
	if(e->pd==3){
        apply_homo_final_ripmap(e->img,img_f,w,h,f->w,f->h,H);
	}else{//suppose pd=1
        float *img3 = malloc(3*w*h*sizeof(float));
        for(int l = 0;l<3;l++){
            for(int i=0;i<w*h;i++){
        ...
        apply_homo_final_ripmap(img3,img_f,w,h,f->w,f->h,H);
	}
	for(int i=0;i<3*(f->w)*(f->h);i++){(f->rgb)[i]=float_to_byte(img_f[i]);}
}
